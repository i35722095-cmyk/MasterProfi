"""Адаптер Anthropic Messages API (Claude) к общему контракту LLMProvider."""
...
class ClaudeProvider:
    """Облачная модель Claude через Anthropic Messages API."""

    def __init__(self, settings: dict[str, Any], logger: Callable[[str], None] = print) -> None:
        self.settings = settings
        self.log = logger
        self.label = str(settings.get("display_name") or settings.get("model") or "Claude")
        api_key_env = str(settings.get("api_key_env", "ANTHROPIC_API_KEY"))
        self.api_key = os.environ.get(api_key_env, "")
        if not self.api_key:
            raise ValueError(f"Не найден API-ключ для {self.label}: задайте переменную окружения {api_key_env}.")
...
Входные записи:\n""" + json.dumps(records, ensure_ascii=False)
    ...
    def _call(self, payload: dict[str, Any]) -> str:
        request = urllib.request.Request(
            self.settings.get("base_url", "https://api.anthropic.com/v1/messages"),
            data=json.dumps(payload, ensure_ascii=False).encode("utf-8"),
            headers={
                "Content-Type": "application/json",
                "x-api-key": self.api_key,
                "anthropic-version": str(self.settings.get("anthropic_version", "2023-06-01")),
            },
        )
